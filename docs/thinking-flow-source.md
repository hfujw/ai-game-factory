...
def tool_design(material: list[dict], user_input: str = "") -> dict:
    """分析素材，决定用什么叙事形式。"""
    if not material:
...
def tool_compose(material: list[dict], design: dict, user_input: str = "") -> dict:
    """写叙事文案+来源标注。"""
    brief = "\n\n".join(
...
输出JSON。thought 字段是你每步决策前的完整内心独白，必须包含：
1. 当前状态回顾（我知道什么、不知道什么）
2. 我的判断（为什么选这个工具而不是别的）
3. 我的预期（做完这步之后会怎样、下一步大概是什么）
至少要写3-4句话，让人感觉你真的在想。好例子："嗯，用户想了解秦始皇修长城。我对这个话题知道一些——长城始建于战国、秦始皇统一后大规模连接扩建、用了大量民夫——但具体用了多少人、花了多少年、工程怎么组织的，这些细节我不太确定。先搜一下，看看有没有具体数字和工程细节。如果素材里时间线清楚，就做时间轴。"
坏例子："决定搜索。"
...
async def orchestrator_node(state: dict) -> dict:
    ...
    while ctx["steps"] < ctx["max_steps"] and ctx["budget_spent"] < ctx["budget_total"]:
        ...
        # 1. 让LLM决定下一步（to_thread 防止同步阻塞卡死事件循环）
        decision = await asyncio.to_thread(_decide, ctx)
        ...
        # 4. 执行工具（to_thread 防止同步阻塞）+ 每 2 秒 pulse 一次
        pulse_done = threading.Event()
        def _pulse():
            while not pulse_done.is_set():
                _time.sleep(2)
                if not pulse_done.is_set() and push:
                    asyncio.run_coroutine_threadsafe(
                        push({"type": "tool_result", "step": ctx["steps"] + 1, "tool": tool_name,
                              "summary": "正在生成…", "budget": ctx["budget_spent"]}),
                        asyncio.get_event_loop()
                    )
        _pulse_thread = threading.Thread(target=_pulse, daemon=True)
        _pulse_thread.start()
        result = await asyncio.to_thread(_execute_tool, tool_name, decision.get("params", {}), ctx)
        pulse_done.set()
...
⚠️ 必须围绕这个主题生成，不要偏离或扩展。用户说"秦始皇修长城"，你就只做修长城，不要做成"秦朝全景"。
...
"""
    ...
    try:
        result = chat(summary, system=ORCHESTRATOR_SYSTEM_PROMPT, temperature=0.5)
        result = _strip_markdown_fence(result)
...
def _execute_tool(tool_name: str, params: dict, ctx: dict) -> dict:
    ...
    if tool_name == "search":
        ...
        result = tool_design(ctx["material"], ctx["user_input"])
        ctx["design"] = result
        return result

    elif tool_name == "compose":
        result = tool_compose(ctx["material"], ctx["design"] or {}, ctx["user_input"])
        ctx["content"] = result
        return result
